...
int main(int argc, char **argv) {
  ...
  if (argc != 2) {
    fprintf(stderr, "usage: resolver.fixture <fixture.ms>\n");
    return 1;
  }
  ...
  if (root != NULL) {
    ...
    (void) ms_resolve_program(argv[1], root, &table, &diagnostics);
  ...
  if (!compare_snapshot("diagnostics", expected_diag_path, &diag_snapshot)) {
    free(source);
    ms_buffer_destroy(&diag_snapshot);
    ms_diag_list_destroy(&diagnostics);
    ms_arena_destroy(&arena);
