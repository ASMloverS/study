...
int main(void) {
  ...
  MsClass *class_object;
  MsInstance *instance_object;
  MsFunction *method_function;
  MsClosure *method_closure;
  MsBoundMethod *bound_method_object;
  MsValue class_value;
  MsValue instance_value;
  MsValue bound_method_value;
  int boolean = 0;
  double number = 0.0;
  MsString *string_value = NULL;
  MsObject first_object;
  MsObject second_object;
  MsClass *out_class = NULL;
  MsInstance *out_instance = NULL;
  MsBoundMethod *out_bound_method = NULL;
  ...
  class_object = ms_class_new("Widget", strlen("Widget"), NULL);
  instance_object = ms_instance_new(class_object);
  method_function = ms_function_new("run", strlen("run"), 0);
  method_closure = ms_closure_new(method_function);
  bound_method_object = ms_bound_method_new(ms_value_object((MsObject *) instance_object),
                                            method_closure);

  TEST_ASSERT(empty_string != NULL);
  TEST_ASSERT(hello_left != NULL);
  TEST_ASSERT(hello_right != NULL);
  TEST_ASSERT(class_object != NULL);
  TEST_ASSERT(instance_object != NULL);
  TEST_ASSERT(method_function != NULL);
  TEST_ASSERT(method_closure != NULL);
  TEST_ASSERT(bound_method_object != NULL);
  ...
  class_value = ms_value_object((MsObject *) class_object);
  instance_value = ms_value_object((MsObject *) instance_object);
  bound_method_value = ms_value_object((MsObject *) bound_method_object);
  ...
  TEST_ASSERT(ms_value_is_class(class_value));
  TEST_ASSERT(ms_value_is_instance(instance_value));
  TEST_ASSERT(ms_value_is_bound_method(bound_method_value));
  ...
  TEST_ASSERT(ms_value_get_class(class_value, &out_class));
  TEST_ASSERT(out_class == class_object);
  TEST_ASSERT(ms_value_get_instance(instance_value, &out_instance));
  TEST_ASSERT(out_instance == instance_object);
  TEST_ASSERT(ms_value_get_bound_method(bound_method_value, &out_bound_method));
  TEST_ASSERT(out_bound_method == bound_method_object);
  ...
  TEST_ASSERT(!ms_value_is_falsey(class_value));
  TEST_ASSERT(!ms_value_is_falsey(instance_value));
  TEST_ASSERT(!ms_value_is_falsey(bound_method_value));
  ...
  TEST_ASSERT(ms_value_equals(class_value, class_value));
  TEST_ASSERT(ms_value_equals(instance_value, instance_value));
  TEST_ASSERT(ms_value_equals(bound_method_value, bound_method_value));
  ...
  TEST_ASSERT(ms_value_equals(ms_bound_method_receiver(bound_method_object),
                              instance_value));
  TEST_ASSERT(ms_bound_method_method(bound_method_object) == method_closure);
  ...
  ms_bound_method_free(bound_method_object);
  ms_closure_free(method_closure);
  ms_function_free(method_function);
  ms_instance_free(instance_object);
  ms_class_free(class_object);
